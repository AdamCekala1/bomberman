...
class Example(QWidget):
        ...
        try:
            ...
            for i in range(0, dimension):
                for j in range(0, dimension):
                    if (str(self.board[i][j]) == str(Player1.char)):
                        if(Player1):
                            # Player1.set_position((i,j))
                            self.user.setGeometry(QtCore.QRect(i * 21, j * 21, 21, 21))
    ...
    def destroy_by_bom(self):
        ...
        for i in self.explX:
            ...
            if(self.board[self.positiony][i]=='0'):
                self.board[self.positiony][i] = '.'
                objectO = self.findChild(QWidget, "wall_type1_"+str(self.positiony)+"_"+str(i))
                objectO.close()
        for i in self.explY:
            if (self.board[i][self.positionx] == '0'):
                self.board[i][self.positionx] = '.'
    ...
    def recieve(self, socket):
        while True:
            resp = socket.recv(64920).decode('utf-8')
    ...
    def create_socket(self):
        ...
        print("hi ", address)
        self.clientsocket.send(jsonpickle.encode(self.board).encode('utf-8'))
    ...
    def keyPressEvent(self, e):
        ...
        if e.key() == Qt.Key_W:
            ...
            if (positiony-1 >= 0):
                nextPosition=self.board[positiony-1][positionx];
                if (nextPosition == '.' ):
                    Player1.move('w')
                    print('------------------')
                    print_board(self.board)
        ...
        if e.key() == Qt.Key_A:
            positionx, positiony = Player1.get_position()
            if (positionx-1 >= 0):
                nextPosition=self.board[positiony][positionx-1]
                if (nextPosition == '.'):
                    Player1.move('a')
                    print('------------------')
                    print_board(self.board)
        ...
        if e.key() == Qt.Key_Escape:
            ...
            self.board[self.positiony][self.positionx] = self.B.char
